...
const config_variables_rec valueTable[] = {
		...
		{ "slp", 				typeFLOAT, "pids", &mainConfig.pidConfig[PITCH].slp, 					0, 25, 10, "" },
		{ "sli", 				typeFLOAT, "pids", &mainConfig.pidConfig[PITCH].sli, 					0, 25, 01, "" },
		{ "sla", 				typeFLOAT, "pids", &mainConfig.pidConfig[PITCH].sla, 					0, 75, 40, "" },
		{ "sld", 				typeFLOAT, "pids", &mainConfig.pidConfig[PITCH].sld, 					0, 0.9, 0.30, "" },
...
void ProcessCommand(char *inString)
{
	...
	if (!strcmp("more", inString))
		...
		{
			snprintf(rf_custom_out_buffer, RF_BUFFER_SIZE, "%lu %lu %lu", deviceWhoAmI, errorMask, failsafeHappend);
			RfCustomReplyBuffer(rf_custom_out_buffer);
		}
